...
if incomplete_rows > 0:
    print(f"  ⚠️  {incomplete_rows} records have missing values - consider imputation")
if outliers.count() > 0:
    print(f"  ⚠️  {outliers.count()} outliers detected - review for data quality")
...
print("="*80)
print("\\nKey Findings:")
print(f"  • Dataset has {df.count()} customers across {df.select('city').distinct().count()} cities")
...
def main():
    ...
    print(f"  Task: Exploratory Data Analysis")
    print()
    ...
    try:
        ...
        if final_status.state != ApplicationState.COMPLETED:
            print(f"  ⚠️  Application did not complete successfully: {final_status.state.value}")
            print("  Check logs below for details.")

    except TimeoutError:
        print(f"  ❌ Application did not complete within 5 minutes")

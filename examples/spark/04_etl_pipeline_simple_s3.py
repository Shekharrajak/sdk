...
def main():
    ...
    try:
        ...
        print(f"  ✓ ETL pipeline submitted successfully!")
    ...
    try:
        ...
        if final_status.state != ApplicationState.COMPLETED:
            print(f"  ⚠️  Pipeline did not complete successfully: {final_status.state.value}")
            print("  Check logs below for details.")

    except TimeoutError:
        print(f"  ❌ Pipeline did not complete within 5 minutes")

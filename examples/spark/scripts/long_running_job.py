...
from pyspark.sql.functions import col, count, sum, avg, min, max, rand, expr
import time
...
def main():
    ...
    spark = SparkSession.builder \
        .appName("Long-Running UI Validation Job") \
        .getOrCreate()
    ...
    df_large = spark.range(0, 100_000_000) \
    ...
    agg_by_category = df_large.groupBy("category") \
        .agg(
    ...
    df_joined = df_large.join(
        dim_categories,
        df_large.category == dim_categories.category_id,
        "inner"
    )
    ...
    df_joined.select("id", "category", "category_name", "department", "value", "region") \
        .show(10)
    print()
    ...
    dept_region_agg = df_repartitioned.groupBy("department", "region") \
        .agg(
    ...
    ranked_regions = dept_region_agg \
    ...
    dept_summary = df_repartitioned.groupBy("department") \
        .agg(
            count("id").alias("transactions"),
    ...
    region_summary = df_repartitioned.groupBy("region") \
        .agg(
            count("id").alias("transactions"),
            sum("value").alias("revenue")
        ) \

...
from kubeflow.spark import SparkClient, OperatorBackendConfig, ApplicationState
...
class TestSparkClientIntegration(unittest.TestCase):
    ...
    def test_04_get_logs(self):
        ...
        print(f"Waiting for driver pod to be ready...")
        is_ready = self.client.wait_for_pod_ready(app_name, timeout=120)

        if not is_ready:
            print("⚠️  Driver pod not ready within timeout, logs may be empty")
    ...
    def test_07_dynamic_allocation(self):
        ...
        print(f"  Config: min=1, max=5, initial=1")

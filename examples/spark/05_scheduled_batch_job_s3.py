...
try:
    from minio_config import get_s3_spark_conf, S3_PATHS, print_minio_info
except ImportError:
...
def main():
    ...
    print(f"  Spark version: 4.0.0")
    print(f"  Resources: 1 driver + 2 executors")
    print(f"  Restart policy: OnFailure (retry up to 3 times)")
    print()
    ...
    try:
        ...
        print(f"  Script loaded from S3: ✓")
        print()
        print("  🌐 Spark UI Access (choose one):")
        print(f"     Option 1 - Direct to driver pod:")
        print(f"       kubectl port-forward pod/{app_name}-driver 4040:4040")
        print(f"     Option 2 - Via service (if created by operator):")
        print(f"       kubectl port-forward svc/{app_name}-ui-svc 4040:4040")
        print(f"     Then open: http://localhost:4040")
        print()
        ...
        print(f"    • Auto-cleanup: After 1 hour")
        print()
    ...
    try:
        ...
        if final_status.state != ApplicationState.COMPLETED:
            print(f"  ⚠️  Job did not complete successfully: {final_status.state.value}")
            print("  Restart policy would trigger automatic retry")
            print("  Check logs below for details.")

    except TimeoutError:
        print(f"  ❌ Job did not complete within 5 minutes")

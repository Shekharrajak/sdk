...
class OperatorBackend(SparkBackend):
    ...
    def __init__(self, config: OperatorBackendConfig):
        ...
        logger.info(
            f"Initialized OperatorBackend with namespace: {self.config.namespace}"
        )
    ...
    def submit_application(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.info(
                f"SparkApplication {target_namespace}/{app_name} created successfully"
            )
    ...
    def delete_application(self, submission_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"SparkApplication {self.config.namespace}/{submission_id} deleted"
            )
    ...
    def _build_restart_policy(self, restart_policy: Optional[Any]) -> Dict[str, Any]:
        ...
        if isinstance(restart_policy, RestartPolicy):
            policy_dict = {
                "type": restart_policy.type.value if isinstance(restart_policy.type, RestartPolicyType) else restart_policy.type
            }
            ...
            if restart_policy.on_submission_failure_retries is not None:
                policy_dict["onSubmissionFailureRetries"] = restart_policy.on_submission_failure_retries
            if restart_policy.on_submission_failure_retry_interval:
                policy_dict["onSubmissionFailureRetryInterval"] = restart_policy.on_submission_failure_retry_interval
            return policy_dict

...
from kubeflow.spark.models import SparkApplicationRequest, RestartPolicyType, DeployMode
...
class ResourceValidator:
    ...
    def validate(self, request: SparkApplicationRequest) -> ValidationResult:
        ...
        if request.num_executors < 1 and not (request.dynamic_allocation and request.dynamic_allocation.enabled):
            result.add_error(ValidationError(
...
class DynamicAllocationValidator:
    ...
    def validate(self, request: SparkApplicationRequest) -> ValidationResult:
        ...
        if dyn_alloc.initial_executors is not None:
            if dyn_alloc.min_executors is not None and dyn_alloc.initial_executors < dyn_alloc.min_executors:
                result.add_error(ValidationError(
                    type=ValidationErrorType.DYNAMIC_ALLOCATION,
            ...
            if dyn_alloc.max_executors is not None and dyn_alloc.initial_executors > dyn_alloc.max_executors:
                result.add_error(ValidationError(
                    type=ValidationErrorType.DYNAMIC_ALLOCATION,
                    field="dynamic_allocation.initial_executors",
...
class SparkApplicationValidator:
    ...
    def validate_and_raise(self, request: SparkApplicationRequest):
        ...
        if not result.valid:
            ...
            raise ValueError(
                f"Spark application validation failed:\n" +
